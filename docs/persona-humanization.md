...
1. **Кто отвечает** — `ChatPersonaDispatchPlanner`. Кандидаты берутся из `bot.persona_chat_bindings`
   ...
   *Новое:* `PersonaAddressResolver` определяет, к кому обращено сообщение — ответ на сообщение
   ...
   Адресат отвечает всегда (без броска `reply_probability`), остальные молчат. Если адресат спит —
   не отвечает никто. Без адресата — каждый кандидат бросает свой `reply_probability`
...
4. **Как это звучит** — `ReplyHumanizer` (через `ResponsePostProcessor`) на всех трёх хендлерах
   (CONCISE / ENHANCED / MULTI_STAGE) и на генерации дайджестов: снимает markdown и списки, убирает
   «Отличный вопрос!», «Надеюсь, это поможет», «Great question» и т.п. на языке ответа, модерирует
   тире, применяет привычки персоны (эмодзи, строчная буква в начале, без точки в конце). Если в тексте
   есть признание в том, что это ИИ, ответ не отправляется (MULTI_STAGE делает одну попытку переписать).
5. **Когда отправить** — `HumanSendPacer` на всех путях отправки (прямой ответ, отложенная очередь,
   ...
   персоны в одном чате, индикатор «печатает…» каждые 4 с на время, пропорциональное длине ответа.
   Отложенная очередь отправляет ответы одной персоны в одном чате строго последовательно.
