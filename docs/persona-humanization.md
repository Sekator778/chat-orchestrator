...
## Проверка отправки и получения (делать после каждой правки reply-пути)

```bash
scripts/test-chat-roundtrip.sh            # первый чат из test.chats
scripts/test-chat-roundtrip.sh <chatId>   # конкретный тестовый чат
```

Скрипт проверяет три вещи подряд и возвращает ненулевой код, если хоть одна не прошла:

1. **отправка** — персона кладёт строку в тестовый чат, Telegram её принимает;
2. **получение** — та же строка возвращается из Telegram и попадает в `bot.messages`;
3. **ответ** — входящее человеческое сообщение проигрывается через Kafka, персона отвечает
   настоящей репликой от модели, и та доходит до Telegram.

Под этим лежат эндпоинты `/api/admin/test-chat` (`send`, `trigger`, `messages`, список чатов).
Любой чат вне списка `bot.app_settings.test.chats` они отклоняют с 403 — опечатка в id не может
отправить тестовое сообщение в живой разговор. Список чатов заводится миграцией cs085 и меняется
без рестарта.

Одно и то же сообщение принимается повторно только через 15 минут: столько живёт ключ
идемпотентности в памяти.

## Как посмотреть состояние на стенде
